	...
	for unitIndex, unit in ipairs(unitsInZone) do
		if unit:getPlayerName() then --if this is nil, it means it's an AI unit and we aren't interested in that.
			...
			if unit:getLife() > 1 and unit:getPosition().p.y <= self.RaceZoneCeiling then	-- Only check for alive units below the racezone ceiling
				...
				if not playerExists then --Player was not already in the race list, so we'll add him after a few more checks...
					...
					if airborne == true then
						...
						if self.GroupRace == true and self.PaceUnitName ~= nil then					
							...
							if distToPace <= self.GroupRaceParticipantFilter then
								--Add the player to the group race list
	...
	for pylonIndex, pylonName in ipairs(self.PylonZones) do
		...
		if #playersInsideZone > 0 and player.PylonFlag == false then
			...
			if  gateAltitudeOk == true then
				warnPlayer(string.format("Pylon hit! Penalty: %s sec.", string.format("%.2f", self.PenaltyTimePylonHit):gsub("%.00$",""):gsub("(%..-)0+$", "%1")), player)
	...
	if self.CustomGateHeights[gateToCheck] then --if a custom gate height definition exists...
		...
		if playerAgl <= self.CustomGateHeights[gateToCheck][2] * unitConversion then
			if playerAgl >= self.CustomGateHeights[gateToCheck][1] * unitConversion then
				...
				result = false
				warnPlayer(string.format("Flew under %s! Altitude = %s %s | Penalty: %s sec.", gateToCheck, string.format("%.2f", playerAgl / unitConversion):gsub("%.00$",""):gsub("(%..-)0+$", "%1"), self.DistanceUnits, string.format("%.2f", self.PenaltyTimeAboveGateHeight):gsub("%.00$",""):gsub("(%..-)0+$", "%1")), player)	
				env.info(string.format("%s flew under %s! Altitude = %s %s | Penalty: %s sec.", player.Name, gateToCheck, string.format("%.2f", playerAgl / unitConversion):gsub("%.00$",""):gsub("(%..-)0+$", "%1"), self.DistanceUnits, string.format("%.2f", self.PenaltyTimeAboveGateHeight):gsub("%.00$",""):gsub("(%..-)0+$", "%1")), player)
			end
		else
			result = false
			warnPlayer(string.format("Flew above %s! Altitude = %s %s | Penalty: %s sec.", gateToCheck, string.format("%.2f", playerAgl / unitConversion):gsub("%.00$",""):gsub("(%..-)0+$", "%1"), self.DistanceUnits, string.format("%.2f", self.PenaltyTimeAboveGateHeight):gsub("%.00$",""):gsub("(%..-)0+$", "%1")), player)
			env.info(string.format("%s flew above %s! Altitude = %s %s | Penalty: %s sec.", player.Name, gateToCheck, string.format("%.2f", playerAgl / unitConversion):gsub("%.00$",""):gsub("(%..-)0+$", "%1"), self.DistanceUnits, string.format("%.2f", self.PenaltyTimeAboveGateHeight):gsub("%.00$",""):gsub("(%..-)0+$", "%1")), player)
		end	
...
function Airrace:CheckGateSpeedForPlayer(player)
	...
	if speed * 3.6 <= self.StartSpeedLimit then
		...
		result = false
		warnPlayer(string.format("Starting speed limit is %d kts! You: %s kts", self.StartSpeedLimit * .54, string.format("%.2f", speed * 3.6 * .54):gsub("%.00$",""):gsub("(%..-)0+$", "%1")), player)
	end
	return result
end
-----------------------------------------------------------------------------------------
	...
	for i = 1 , #self.HorizontalGates do
		if self.HorizontalGates[i] == gateNumber then			
			if roll >= -10 and roll <= 10 then
				...
				result = false
				warnPlayer(string.format("Wings not level! Roll = %s deg. | Penalty: %s sec.", string.format("%.2f", roll):gsub("%.00$",""):gsub("(%..-)0+$", "%1"), string.format("%.2f", self.PenaltyTimeHorizontalGate):gsub("%.00$",""):gsub("(%..-)0+$", "%1")), player) --this formating trims off decimal places if they are not needed
			end
	...
	for i = 1 , #self.VerticalGates do
		if self.VerticalGates[i] == gateNumber then
			if (roll >= 80 and roll <= 100) or (roll >= -100 and roll <= -80) or (roll >= 260 and roll <= 280) then
				...
				result = false
				warnPlayer(string.format("Wings not vertical! Roll = %d deg. | Penalty: %d sec.", string.format("%.2f", roll):gsub("%.00$",""):gsub("(%..-)0+$", "%1"), string.format("%.2f", self.PenaltyTimeVerticalGate):gsub("%.00$",""):gsub("(%..-)0+$", "%1")), player)
			end
			if result == false then
				trigger.action.outSoundForUnit(player.UnitID, 'penalty.ogg')
	...
	for i = 1 , #self.InvertedGates do
		if self.InvertedGates[i] == gateNumber then
			if (roll >= 170 and roll <= 190) or (roll <= -170 and roll >= -190) then
				...
				result = false
				warnPlayer(string.format("Wings not level while inverted! Roll = %d deg. | Penalty: %d sec.", string.format("%.2f", roll):gsub("%.00$",""):gsub("(%..-)0+$", "%1"), string.format("%.2f", self.PenaltyTimeInvertedGate):gsub("%.00$",""):gsub("(%..-)0+$", "%1")), player)
			end
			if result == false then
				trigger.action.outSoundForUnit(player.UnitID, 'penalty.ogg')
...
function Airrace:CheckLineupWithPace(player)
	...
	elseif playerPos.y < -152.4 then -- 152.4 meters = 500 feet
		env.info(string.format("Player %s is not eligible for the current group race because they are more than 500 ft below the pace plane", player.Name))
		warnPlayer(string.format("(More than %d %s below the Pace)", math.floor(152.4/unitConversion), self.DistanceUnits), player)
		player.RaceEligible = false 
	end    
...
function Airrace:ListPlayers()
	...
	if self.GroupRace == true then
		...
		--if there's no pace plane present...	
		else 
			...
			if self.PaceUnitName ~= nil then
				...
				if paceDroppedIn == 1 then -- 1=true in DCS user flag
					...
					if trigger.misc.getUserFlag("Lap1Gate1Reached") == 1 then
						local lapNumberCorrection = (self.NumberLaps == 0) and 1 or self.NumberLaps
						...
						trigger.action.setUserFlag("GroupRaceFinalLap", 0)
					end
				end
			end			
		...
		--for group races that have started...
			...
			for currentRank, playerData in ipairs(rankTable) do
                if self.NumberLaps > 1 and playerData.lap > highestLapNum then
					...
					if highestLapNum == self.NumberLaps then
						trigger.action.setUserFlag("GroupRaceFinalLap", 1) --optional flag to be used in the .miz for whatever purpose
					end
                end
			end
			if trigger.misc.getUserFlag("GroupRaceFinished") == 0 then
...
function Init()
	...
	local groupRaceEnforceAirspace = GroupRaceEnforceAirspace
	...
	if type(groupRaceEnforceAirspace) ~= "boolean" then
		groupRaceEnforceAirspace = false
		env.info("Invalid or missing setting for groupRaceEnforceAirspace. Must be true or false. Resetting to default: false")
	end
...
env.info("-----------------------------------------------------------------------------------------")
env.info("Load AirRaceScript4.6")
